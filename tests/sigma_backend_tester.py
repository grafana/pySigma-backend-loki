...
parser.add_argument('signature_path',
        ...
        try:
            ...
            if args.summarize:
                for rule in sigma_rules:
                    fields = list(item.field
                            for detection in rule.detection.detections.values()
                            for item in find_all_detection_items(detection, [])
                            if item.field is not None) + rule.fields
                    for field in fields:
            ...
            if args.unique:
                ...
                counters['error_messages'][str(err)] = counters['error_messages'].get(str(err), 0) + 1
            return
        try:
...
def print_counts(dct):
    if len(dct) == 0:
        print("\tNo results")
        return
    for k, v in collections.OrderedDict(sorted(dct.items(), key=operator.itemgetter(1), reverse=True)).items():
        print(f"\t{k}: {v}")
...
if args.counts:
    ...
    print(f"Successfully converted {counters['convert_success']} out of {counters['total_sigs']} ({percent_conv:.2f}%) signatures")
    if args.validate:
        percent_valid = counters['validate_success'] / counters['convert_success']* 100
        print(f"Successfully validated {counters['validate_success']} out of {counters['convert_success']} ({percent_valid:.2f}%) signatures")
